...
def test_page_footnote_uses_separator_and_stops_before_distant_footer_text() -> None:
    ...
    pdf_extractor._classify_page_auxiliary_text(page)
...
def test_page_footnote_supports_independent_column_rules() -> None:
    ...
    page = _prepared_text_page(*lines, page_size=(1000.0, 1000.0))
    page.drawing_lines = [
        pdf_extractor._AxisLine((80.0, 800.0, 220.0, 802.0), 1.0, "horizontal"),
        pdf_extractor._AxisLine((540.0, 800.0, 680.0, 802.0), 1.0, "horizontal"),
    ]
...
def test_page_footnote_unions_aligned_regular_and_span_lanes_only() -> None:
    ...
    page = _prepared_text_page(*lines, page_size=(1000.0, 1000.0))
    page.drawing_lines = [
        pdf_extractor._AxisLine((100.0, 800.0, 305.0, 802.0), 1.0, "horizontal")
    ]
...
def test_page_footnote_entries_split_first_line_indent_without_text() -> None:
    ...
    entries = pdf_extractor._split_page_footnote_entries(
        lines,
        (595.0, 842.0),
    )
...
def test_page_footnote_entries_split_hanging_indent_and_tighten_boxes() -> None:
    ...
    blocks = [
        block
        for block in pdf_extractor._finalize_prepared_page(page, page_index=0)
...
def test_page_footnote_rejects_decorative_formula_table_and_missing_rules(
    ...
    if rule_bbox is not None:
        page.drawing_lines = [pdf_extractor._AxisLine(rule_bbox, 1.0, "horizontal")]
...
def test_page_footnote_rejects_collinear_rule_segment_next_to_table() -> None:
    ...
    page.drawing_lines = [
        pdf_extractor._AxisLine((100.0, 750.0, 260.0, 752.0), 1.0, "horizontal"),
        pdf_extractor._AxisLine((280.0, 750.0, 900.0, 752.0), 1.0, "horizontal"),
    ]
...
def test_aside_text_rejects_short_internal_wide_and_non_dominant_rotated_text() -> None:
    ...
    pdf_extractor._classify_page_auxiliary_text(rotated_page)
...
def test_auxiliary_text_classifiers_do_not_read_line_text() -> None:
    ...
    source = "\n".join(
        ...
        for function in (
            pdf_extractor._classify_page_auxiliary_text,
            pdf_extractor._classify_aside_text,
            pdf_extractor._geometric_text_support_by_angle,
            pdf_extractor._classify_page_footnotes,
            pdf_extractor._rule_belongs_to_confirmed_table,
            pdf_extractor._merge_overlapping_source_groups,
            pdf_extractor._footnote_lane_members,
            pdf_extractor._split_page_footnote_entries,
            pdf_extractor._tight_page_footnote_bboxes,
        )
    )
...
def test_finalize_preserves_preclassified_auxiliary_text_types() -> None:
    ...
    blocks = pdf_extractor._finalize_prepared_page(page, page_index=0)
