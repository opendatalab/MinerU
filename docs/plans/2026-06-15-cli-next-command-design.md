...
5. **暂不增加内容读取子命令**
   - 暂不加入 `mineru content <sha256> --tier basic --pages 1~5`。
   - 暂不加入 `mineru show doc <sha256> --content --tier basic --pages 1~5`。
...
```bash
mineru parse <file> [--tier flash|basic|standard] [--pages <range>] [--format markdown] [--force] [--remote] [--wait <sec>] [--no-wait] [-o <path>] [--json] [-v]
```
...
```bash
mineru list parses [--status pending|parsing|done|failed|superseded] [--tier flash|basic|standard] [--limit 50] [--offset 0] [--json]
```
...
- 用于查看单个 parse task 的详细状态、错误、tier、page_range、关联 sha256、产物路径等。
- 不读取正文内容。
...
```bash
mineru list scans [--status pending|running|done|failed] [--kind manual|watch] [--watch-id <id>] [--limit 50] [--offset 0] [--json]
```
...
- `show file <path>` 需要保留当前 `get_file_by_path()` 的行为：在返回信息前先执行 `ensure_ingested(path)`，刷新 path 对应的 file/doc 状态。
...
- 仅展示 doc metadata、关联路径、已解析 tier/page_range、active parses、FTS 状态。
- 不加 `--content`。
- 内容读取命令暂不纳入本轮设计。
...
```bash
mineru list docs [--file-type <type>] [--limit 200] [--offset 0] [--json]
```

说明：

- `list docs` 返回 active docs，以 sha256 去重；active docs 指当前至少被一个 `status=active` 的 file row 引用的 doc。
- 支持按 `docs.file_type` 过滤，例如 `pdf`、`docx`。
- `list docs` 不展示 orphan docs，也不展示只被 `deleted` / `unreachable` file row 引用的 docs。这些状态属于 cleanup / maintenance 视图。
- `list docs` 不支持 `--path`。path 在当前模型中唯一映射到一个 active file 和最多一个 doc，需要按路径取 doc 时使用 `GET /docs/by-path` / `DoclibClient.get_doc_by_path(path)`。
...
```bash
mineru content <sha256> --tier basic --pages 1~5
mineru show doc <sha256> --content --tier basic --pages 1~5
mineru parse list
mineru parses ...
mineru scan list
...
- `mineru list scans` 调用现有 `DoclibClient.list_scans()`，并透传 `limit` / `offset`。
- `mineru list docs` 调用 `DoclibClient.list_docs()`，并补齐 `file_type` / `limit` / `offset` 参数。
- `mineru list files` 需要新增 server/client/interface endpoint。
- `mineru show scan <id>` 调用现有 `DoclibClient.get_scan()`。
- `mineru list parses` 调用现有 `DoclibClient.list_parses()`，并补齐 `limit` / `offset` 参数透传。
- `mineru show parse <id>` 调用现有 `DoclibClient.get_parse()`。
- `mineru show file <path>` 复用现有 `get_file_by_path()`，并保留其 `ensure_ingested(path)` 行为。
- `mineru show doc <sha256>` 调用现有 `DoclibClient.get_doc(expand_files=True)`。
- SDK/API 如需按路径直接获取当前 doc，使用 `DoclibClient.get_doc_by_path(path)` / `GET /docs/by-path?path=...`。
