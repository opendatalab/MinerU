...
镜像从包索引安装区间 `mineru[torch]>=4.0,<5`，**不会**安装作为构建上下文的仓库源码；区间安装不随镜像 tag 锁定。即使从开发分支或打过补丁的 checkout 构建，得到的仍是包索引版本镜像；之后用同一 tag 重建也可能装到其他 4.x 版本。两条路径对应不同需求：

| 路径 | 版本来源 | 适用场景 |
| --- | --- | --- |
| 正式版镜像（`docker/global/Dockerfile`、`docker/china/Dockerfile`） | 包索引区间 `mineru[torch]>=4.0,<5` | 生产部署 |
| 源码调试容器（正式版镜像 + 挂载 checkout） | 你的工作副本 / 提交 | 验证开发分支、调试、复现补丁 |
