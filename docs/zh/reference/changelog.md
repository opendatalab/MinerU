...
- 破坏性变更：移除 `mineru.parser` 公开导出中的 `PARSER_BACKENDS` 常量（4.0 正式版包含该导出）。backend 由 tier 派生，不再作为公开选择；如需某 tier 的实现名称，请调用 `mineru.parser.backend_for_tier(tier)`。

## 4.0

- 四档解析、多格式原生文档支持、文档库与渐进阅读。
- ONNX/Torch 小模型与独立 VLM 引擎配置，统一 SDK、V1 API、WebUI 和渲染接口。
- 主要能力见[4.0 概览](../index.md)，接口与部署调整见[迁移指南](migration_4.md)；旧平台适配继续限制 `mineru<4`。
...
- 通过支持多个pdf文件的batch处理（[脚本样例](https://github.com/opendatalab/MinerU/blob/ae9fd9adaa336c171938f0ccd509cdf13a09087b/demo/batch_demo.py)），提升了批量小文件的解析速度 (与1.0.1版本相比，公式解析速度最高提升超过1400%，整体解析速度最高提升超过500%)
- 通过优化mfr模型的加载和使用，降低了显存占用并提升了解析速度(需重新执行[模型下载流程](https://github.com/opendatalab/MinerU/blob/magic_pdf-1.1.0-released/docs/how_to_download_models_zh_cn.md)以获得模型文件的增量更新)
