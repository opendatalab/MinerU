...
本计划已经结合当前代码做过一轮快速审计，但不是完整逐行 review。已检查的关键路径:
...
| `mineru/doclib/routes/parse.py` | 现有 `/parse/content` 已能从 JSON 批次读取并即时 render Markdown；NEXT 目标路径为 `GET /docs/{sha256}/content`。 |
| `mineru/doclib/core/fts.py`、`services/search_svc.py` | FTS 已存 tier，搜索结果已返回来源 tier。 |
| `mineru/doclib/client.py` | 已有 Doclib SDK 和 error envelope -> `MineruError` 基础映射。 |
| `mineru/parser/base.py` | `ParseResult.from_dict()` / `from_json()` 仍是 TODO。 |
| `mineru/parser/__init__.py` | Tool SDK `parse()` 已有 backend 参数，但还没有 tier 参数。 |
| `mineru/constants.py` | `Tier` 目前只有 `flash` / `standard` / `pro` 枚举。 |
...
| M3 | tier、privacy、parse-server 路由 | 默认选择、local/remote、standard/pro 行为可验证。 |
| M4 | CLI / SDK / API 对齐 | 用户入口使用同一语义和错误模型。 |
| M5 | Agent-native 输出 | locator、chunk id、marker、citation 基础可用。 |
...
|------|--------------|----------------|
| M0-001 | 测试骨架不足 | 新增测试基础设施。 |
| M0-002 | 已实现需锁定 | 补测试证明 doclib 只落 JSON。 |
...
| M2-001 | 部分实现 | 抽出并统一 batch JSON 路径 helper。 |
| M2-002 | 部分实现 | 修正 cache check 硬编码 data_dir 的风险。 |
| M2-003 | 已实现需锁定 | 补测试锁定未覆盖页入队和 priority bump。 |
| M2-004 | 已实现需锁定 | 补 compaction JSON 合并测试。 |
| M2-005 | 部分实现 | `/parse/content` 已支持 Markdown；补完整 read-time render 边界。 |
| M2-006 | 已实现需锁定 | 补 FTS 不落盘 Markdown 和 tier-gated 测试。 |
| M3-001 | 部分实现 | `tier=None` 已解析；需锁定默认选择不持久化为独立 tier。 |
| M3-002 | 已实现需锁定 | 补 local 不自动 remote 的测试。 |
| M3-003 | 已实现需锁定 | 补 remote fallback local 的测试。 |
| M3-004 | 已实现需锁定 | 补 tier mismatch 不降级的测试和错误 code 检查。 |
| M4-001 | 部分实现 | CLI/doclib 需对齐 read-time render 多格式。 |
| M4-002 | 需要实现 | Tool SDK `parse()` 还没有 `tier` 参数。 |
| M4-003 | 部分实现 | Doclib client 已有映射；API-backed parser 等仍需统一。 |
| M4-004 | 部分实现 | api_server/client 已有基础，需按文档补差异测试。 |
| M5-001 | 需要实现 | 新增 locator/chunk id helper。 |
| M5-002 | blocked | 依赖 M5-001。 |
| M5-003 | 已实现需锁定 | 搜索结果已返回 tier；补测试和 CLI/JSON 边界。 |
...
- [Middle JSON 当前标准](middle-json/current-standard.md)
...
1. 在 schema 或 middle_json helper 模块中定义 `normalize_middle_json(payload, *, backend=None, tier=None, sha256=None)`。
2. 如果 payload 已有 `schema_version` 和 `pages`，补齐缺省 `_meta`。
...
- FTS 有内容。
- Markdown 不落盘。
- tier-gated 更新符合 `flash < standard < pro`。
...
1. 构造 health: local supported tiers = `["standard", "pro"]`。
2. 请求 `tier=None`。
3. 期望入队 tier 为 `pro`。
4. 构造 health: local supported tiers = `["standard"]`。
5. 期望入队 tier 为 `standard`。
...
1. mock parse-server supported tiers = `["standard"]`。
2. 请求 `tier="pro"`。
3. 期望 `tier_mismatch`。
4. 确认不会改成 `standard`。
5. 确认不会改成 `flash`。
...
5. `tier=standard` 映射到默认 standard backend。
6. `tier=pro` 映射到默认 pro backend。
7. `tier=None` 在 Tool SDK 场景中如果不能发现 standard/pro，应报错，不静默 flash。

完成边界:

- `parse(path, tier="flash")` 可用。
- `parse(path, tier="standard")` 可用或返回明确能力错误。
...
### M5-001 定义 locator 和 chunk id helper

状态: `ready`

目标:

为 Agent citation 提供稳定定位基础。
...
- `mineru/schema/` 或 `mineru/types.py`
- `tests/...`

具体步骤:

1. 定义 `locator_for_block(page, block) -> str`。
2. 定义 `locator_for_span(page, block, span) -> str`。
3. 定义 `chunk_id(meta, locator) -> str`。
4. chunk id 输入包含 schema major、sha256、tier、page_idx、locator。
5. 不把文件路径或文件名放入 chunk id。
6. unknown bbox 输出 `bbox_known=false`。

完成边界:

- 同一 fixture 多次生成 locator 稳定。
- 不同 tier 的同一 locator 生成不同 chunk id。
- chunk id 不包含明文路径。

验证方式:

```bash
.venv/bin/python -m pytest tests -q -k "locator or chunk_id"
```
...
3. marker 包含 locator 和 chunk id。
4. 默认不输出 marker。
...
| doclib JSON behavior | M0-002、M2-001、M2-002 |
| privacy / tier tests | M3-001、M3-002、M3-004 |
| CLI / SDK | M4-001、M4-002，需避免同文件冲突 |
| Agent locator | M5-001 可在 M1 validator 稳定后开始 |

不建议并行:

- M2-001 与 M2-002 同时改 batch path。
...
| PR | 包含任务 | 目标 |
|----|----------|------|
| PR-1 | M0-001, M0-002, M0-003 | 测试骨架和 fixture。 |
| PR-2 | M1-001, M1-002, M1-003 | Middle JSON normalize / round-trip / validate。 |
| PR-3 | M2-001, M2-002, M2-003, M2-004 | doclib JSON 批次、cache coverage、compaction。 |
...
| PR-7 | M5-001, M5-002, M5-003 | Agent-native locator / marker / search tier。 |
...
10. workflows 中 13.1 到 13.7 的端到端验收可以通过测试或手工脚本验证。

P1 完成需要满足:

1. 搜索结果返回来源 tier。
2. Agent 主动读取默认升级到 `standard` / `pro`。
3. locator 和 chunk id 可生成。
4. Markdown 可选输出 locator marker。
5. 高质量解析完成后刷新搜索索引。
