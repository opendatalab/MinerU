...
- 质量优先：PDF/image 未指定 tier 时使用默认选择策略，不会解析为 `flash`；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 未指定 tier 时归一为 `flash`。
- 文本直读：`.txt`、`.md`、`.markdown`、`.rst` 和 `.tex` 无需解析，应直接读取源文件；`.csv`/`.tsv` 按本地 flash 结构化解析。
...
5. 未指定 tier：PDF/image 使用默认选择策略，不可降级到 `flash`；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`。
