...
- 质量优先：PDF/image 未指定 tier 时使用默认选择策略，不会解析为 `flash`；OFD/EPUB/Office/HTML/CSV/TSV 未指定 tier 时归一为 `flash`。
...
5. 未指定 tier：PDF/image 使用默认选择策略，不可降级到 `flash`；OFD/EPUB/Office/HTML/CSV/TSV 归一为 `flash`。
