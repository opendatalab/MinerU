...
`mineru-kit api-server` 是正式的 self-hosted parse-server 启动入口。当前一个进程可以通过重复 `--tier` 暴露一个或多个 tier；不传 `--tier` 时暴露 `flash`、`basic`、`standard`、`advanced`。
...
|---------------------|--------------------|
| 包含 `standard` | `standard` |
| 不包含 `standard`，但包含 `advanced` | `advanced` |
| 不包含 `standard` / `advanced`，但包含 `basic` | `basic` |
| 只包含 `flash` | 返回 `quality_tier_unavailable`，除非请求显式传 `tier=flash` |
| 未传 `--tier` | 暴露 `flash`、`basic`、`standard`、`advanced`，默认 `standard` |
...
mineru-kit api-server --tier basic --tier standard --port 8000
