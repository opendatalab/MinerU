...
- Agent 或用户主动读取 PDF/image 文档时，应通过 `mineru parse` 使用默认选择策略或显式 tier；OFD/EPUB/Office/HTML/MHTML/CSV 读取按 [ADR-0024](../decisions/0024-file-type-tier-normalization.md) 与 [ADR-0028](../decisions/0028-csv-structured-flash-parsing.md) 归一为 `flash` 语义；其它 text 直接读取源文件。
...
parsing-rule 的 `tier` 和 `remote` 适用于 PDF 和 image，但 `page_range` 只适用于 PDF。image 忽略 page_range 并整文件解析；EPUB、Office、HTML、MHTML、CSV 和 TSV 命中规则时忽略 tier、page_range 和 remote，按 `flash` 整本解析，并且 parse row 记录为 `flash`；其它 text 只入库和索引，不创建 parse row。
