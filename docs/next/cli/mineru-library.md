...
parsing-rule 的 `tier` 和 `remote` 适用于 PDF 和 image，但 `page_range` 只适用于 PDF。image 忽略 page_range 并整文件解析；EPUB、Office、HTML、CSV 和 TSV 命中规则时忽略 tier、page_range 和 remote，按 `flash` 整本解析，并且 parse row 记录为 `flash`；其它 text 只入库和索引，不创建 parse row。
