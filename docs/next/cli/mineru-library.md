...
parsing-rule 的 `tier`、`page_range` 和 `remote` 只适用于 PDF 和 image。EPUB、Office、HTML 和 CSV 命中规则时忽略这些字段，按 `flash` 整本解析，并且 parse row 记录为 `flash`；其它 text 只入库和索引，不创建 parse row。
