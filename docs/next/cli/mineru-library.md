...
- Agent 或用户主动读取 PDF/image 文档时，应通过 `mineru parse` 使用默认选择策略或显式 tier；Office/HTML/CSV 读取按 [ADR-0024](../decisions/0024-file-type-tier-normalization.md) 与 [ADR-0028](../decisions/0028-csv-structured-flash-parsing.md) 归一为 `flash` 语义；其它 text 直接读取源文件。
...
parsing-rule 的 `tier` 和 `remote` 只适用于 PDF 和 image。Office、HTML 和 CSV 命中规则时忽略 `tier` 和 `remote`，按 `flash` 解析，并且 parse row 记录为 `flash`；其它 text 只入库和索引，不创建 parse row。
