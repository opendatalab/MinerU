...
- PDF/image 的默认选择策略不能降级为 `flash`；找不到非 `flash` 质量 tier 时必须报错。EPUB/Office/HTML/CSV 这类仅支持 flash tier 的输入归一规则见 [ADR-0024](decisions/0024-file-type-tier-normalization.md) 与 [ADR-0028](decisions/0028-csv-structured-flash-parsing.md)。其它文本文件无需解析，显式 parse 请求返回 `parse_not_required`。
...
| `invalid_request_error` | `remote_unsupported_for_file_type` | 400 | 否 | `remote` | EPUB/Office/HTML/CSV 单文件主动解析请求 remote | `use_local_flash_or_choose_pdf_image` |
| `invalid_request_error` | `tier_unsupported_for_file_type` | 400 | 否 | `tier` | EPUB/Office/HTML/CSV 单文件主动解析显式请求质量 tier | `use_tier_flash_or_choose_pdf_image` |
