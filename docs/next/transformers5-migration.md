...
本分支只支持 `transformers>=5.10.1,<6`，回归锚点为 5.10.1 与 5.16.1。
...
| MLX-VLM | `>=0.3.12,<0.4`，MLX 保留 `<=0.31.1` |
...
utils 2.0.0 正式发布前，先构建两个工作树的 wheel，并在独立环境中一起安装，避免解析到旧版 utils：
...
安装矩阵直接读取已构建 wheel 的元数据，覆盖三个平台、Python 3.10–3.13 与基础/torch/full/all 模式：
...
手动触发时可传入 `utils-wheel-url` 验证未发布的联合构建。utils 仓库也提供客户端双版本回归。
