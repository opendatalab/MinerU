...
- 只删除 `scan_status='deleted'` 的 file row。
...
拒绝。`fts_filenames` 的生命周期应与 `files` row 一致。默认 find 通过 `scan_status='active'` 控制可见性即可。
