...
- 可插拔设备拔出与恢复。
- `get_file_by_path(path)`、`parse(path)`、`get_doc_by_path(path)`、`invalidate(path=...)` 等同步 path 操作。
...
|------|------|-----------|----------------|--------------|
| `active` | 路径当前存在且可访问 | 是 | JSON 返回；CLI 优先展示 | 是 |
| `unreachable` | 所属 removable watch 当前不可达 | 否 | JSON 返回；CLI 无 active file 时展示 | 是 |
| `deleted` | 路径已确认不存在，且不是设备整体不可达导致 | 否 | JSON 返回；CLI 无 active file 时展示 | 是 |
...
3. 所有同步涉及 source file path 的 4 个操作:
   ...
   - `GET /files/by-path?path=...` / SDK `get_file_by_path(path)` / `mineru show file <path>`。
   - `GET /docs/by-path?path=...` / SDK `get_doc_by_path(path)`。
...
- `search` JSON 返回文档关联的全部 file aliases 及状态；非 JSON CLI 存在 active files 时只展示 active paths，否则展示全部 inactive paths。
...
部分拒绝。第一版默认 find 只返回当前可访问文件。search JSON 返回全部 aliases，使调用方能看到 active、unreachable 和 deleted 状态；非 JSON CLI 使用 active 优先展示。完全没有 file row 的已索引文档仍作为 orphan 搜索结果返回。
