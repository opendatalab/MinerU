...
- watch 发现变化与用户主动 `parse(path)`、`info(path)` 的行为是否一致。
...
`IngestWorker` 只处理 `scan_status=active AND sha256 IS NULL` 的文件。
...
2. 如果 watch root 不可达，只更新 `watch_targets.watch_status=unreachable`，不逐个刷新该 watch 下的所有 files。
3. 如果 watch root 可达，先读取该 watch 下 `scan_status=active` 的已知 file paths，逐个调用统一发现方法；这一步用于发现已删除文件。
...
- `PermissionError` 不表示 deleted，应写入 `files.error_code=file_permission_denied` 和 `files.error_msg`，保留当前 `scan_status`。
- 其他 stat `OSError` 不表示 deleted，应写入 `files.error_code=stat_failed` 和 `files.error_msg`，保留当前 `scan_status`。
...
- 允许立即把当前 `watch_targets.watch_status` 更新为 `unreachable`，并写入 `unreachable_at`。
...
| 文件信息 | `GET /info?path=...` / `get_file_info(path)` | `mineru info <path>` | 判断 path 是否变化；变化后可返回 `sha256=None`，表示等待重新入库 |
| 按路径列文档 | `GET /docs?path=...` / `list_docs(path=...)` | 后续可由 CLI 暴露 | 判断 path 是否变化；避免返回旧 `sha256` 对应 doc |
...
`info(path)` 也必须先执行统一发现方法。

原因:

- `info` 是用户显式以 path 查询当前文件状态的入口。
- 如果 path 内容或元数据已经变化，返回旧 `sha256`、旧 size 或旧 parse 状态会误导用户和 Agent。
- `info` 只需要做轻量 stat/discover；如果发现变化，清空 `files.sha256` 并让 ingest worker 重新处理。它不需要同步完成 SHA256 计算和 parse。

因此 `info(path)` 的返回可以出现 `file.sha256=None`、`doc=None`、`active_parses=[]`，表示该路径刚被发现为变化状态，正在等待重新入库。
...
### 方案 A: 每次 parse/info 都重新计算 SHA256

拒绝。SHA256 对大文件成本较高，且 `info` 作为状态查询不应默认产生昂贵 IO。
...
- 主动 parse 和 info 需要先 refresh/discover path，再读取 `files.sha256`。
...
4. 修改 `get_file_info(path)`，在查询返回前调用 `discover_file()`。
