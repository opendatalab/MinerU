...
此前 `agent-gaps.md` 讨论过 hash 型 `chunk_id`。本 ADR 将 P0 的主引用模型收敛为局部 locator 与全局 block reference。
...
### 方案 E: 使用 hash 型 chunk id 作为主引用

暂缓。hash 型 chunk id 可作为后续补充字段，但 P0 主引用使用可读的 `block_ref`。
