...
本 ADR 补充 ADR-0014 的 Markdown 图片引用和 Office block image 读取语义。ADR-0014 中 locator-first、PDF bbox 裁图、`DocContentResponse` 与 CLI `--output` 的其他决策保持不变。

## 背景

doclib 的目标是保存可渐进读取的 Middle JSON，而不是为大量 PDF 长期保存所有裁图。当前默认解析路径中，PDF 通常只保存 Middle JSON；Office 因缺少可靠 bbox，会保存解析得到的图片 sidecar。
...
![Table block](doc:ab12cd3/tier:high/page:1/block:5)
![Chart block](doc:ab12cd3/tier:high/page:1/block:6)
![Formula block](doc:ab12cd3/tier:high/page:1/block:7)
```
...
```bash
mineru read "doc:ab12cd3/tier:high/page:1/block:4" --format image --output image.jpg
```
...
1. PDF block 有有效 bbox 时，输出 canonical block locator。渲染 Markdown 时不检查源 PDF 是否存在。
...
1. PDF block 有有效 bbox 时，从当前可访问的源 PDF 即时裁剪。
...
- PDF 源文件不可访问: `no_accessible_file`。
...
Doclib 使用专门的 block renderer 生成这些引用。通用 renderer 不改变，避免影响 Parse SDK、Parse API、`mineru-kit` 和其他直接消费 ParseResult/Middle JSON 的调用方。
...
- PDF 继续避免长期保存大量裁图，只在显式 image read 时按 bbox 生成。
...
- PDF block 有有效 bbox 时输出非空 locator，并可通过同一 locator 裁图。
- PDF block 无有效 bbox，但存在 sidecar 时使用 sidecar。
...
- 让 doclib Markdown renderer 使用类型化虚拟 block 图片引用。
