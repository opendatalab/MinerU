...
- text 输入可入库和建立搜索索引，但无需解析，不产生 parse row 或 Middle JSON；CSV 已由 ADR-0028 移出该分组。
...
2. doclib 单对象主动解析中，`--remote` 与 Office / HTML / CSV 冲突时，返回 `remote_unsupported_for_file_type`。
3. PDF / image 的 `--remote --tier flash` 冲突时，返回 `tier_unsupported_for_remote`。
4. doclib 单对象主动解析中，Office / HTML / CSV 的质量 tier 冲突时，返回 `tier_unsupported_for_file_type`。
...
- Office、HTML、CSV 没有对应 PDF 质量后端能力，text 则无需解析。
...
- 这是有意的 doclib 单对象主动解析行为收紧。依赖 Office / HTML / CSV 显式请求质量 tier 或 `--remote` 成功退出的脚本需要调整；依赖其它 text parse 成功退出的脚本应改为直接读取源文件。
- 未显式指定质量 tier 的 Office、HTML、CSV 解析仍保持 `flash` 语义。
...
- doclib 单对象主动解析中，Office / HTML / CSV 请求质量 tier 报 `tier_unsupported_for_file_type`。
- doclib 单对象主动解析中，Office / HTML / CSV 请求 `--remote` 报 `remote_unsupported_for_file_type`。
...
2. 如果未来其它 text 需要结构化解析，应像 ADR-0028 一样通过新 ADR 更新能力矩阵。
