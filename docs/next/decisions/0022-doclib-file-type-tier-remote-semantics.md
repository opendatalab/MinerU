...
| text (`txt` / `md` / `markdown` / `csv` / `rst` / `tex`) | 无需解析 | 无需解析 | 无需解析 | 无需解析 |

当 Office / HTML 在 doclib 单对象主动解析中显式请求质量 tier 时，doclib 不再自动降级为 `flash`，而是返回 `InvalidRequestError`。批量、parsing-rule、API Server 和 mineru-kit 的归一化例外见 [ADR-0024](0024-file-type-tier-normalization.md)。

- `code`: `tier_unsupported_for_file_type`
- `param`: `tier`

未显式传入 tier 的 Office / HTML 文件继续按 `flash` 语义处理。text 文件无论是否指定 tier，均返回 `parse_not_required`，`param` 为 `path`，提示调用方直接读取源文件。
...
当 Office / HTML 在 doclib 单对象主动解析中请求 `--remote` 时，doclib 返回 `InvalidRequestError`：
...
1. text 文件无需解析，优先返回 `parse_not_required`，不再校验 tier、remote 或 force 的组合。
2. doclib 单对象主动解析中，`--remote` 与 Office / HTML 冲突时，返回 `remote_unsupported_for_file_type`。
3. PDF / image 的 `--remote --tier flash` 冲突时，返回 `tier_unsupported_for_remote`。
4. doclib 单对象主动解析中，Office / HTML 的质量 tier 冲突时，返回 `tier_unsupported_for_file_type`。
...
- Office、HTML 没有对应 PDF 质量后端能力，text 则无需解析。
...
- 这是有意的 doclib 单对象主动解析行为收紧。依赖 Office / HTML 显式请求质量 tier 或 `--remote` 成功退出的脚本需要调整；依赖 text parse 成功退出的脚本应改为直接读取源文件。
- 未显式指定质量 tier 的 Office、HTML 解析仍保持 `flash` 语义。
- text 文件仍可入库、发现和搜索，但显式 parse 返回 `parse_not_required`。
...
- doclib 单对象主动解析中，Office / HTML 请求质量 tier 报 `tier_unsupported_for_file_type`。
- doclib 单对象主动解析中，Office / HTML 请求 `--remote` 报 `remote_unsupported_for_file_type`。
- text 文件的显式 parse 请求（包括 `--force`、quality tier 和 `--remote`）均报 `parse_not_required`，且保留入库和搜索索引。
- PDF / image 请求 `--remote --tier flash` 报 `tier_unsupported_for_remote`。

## 后续动作

1. 在 CLI / tier / error 文档中同步文件类型、tier、remote 的能力矩阵。
2. 如果未来 Office / HTML 支持质量 tier，或 text 需要结构化解析，应通过新 ADR 或本 ADR 的 superseding 记录更新能力矩阵。
