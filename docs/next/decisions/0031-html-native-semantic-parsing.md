...
- 共享 DocVortex renderer 输出版本化 `data-docvortex-html-version="1"` 机器契约。只有当前 renderer 能生成的 canonical
  marker 树进入精确路径，parser 跳过 `auto` 裁剪并按 `data-block-type` 恢复顶层 block、visual child、列表/目录叶子及关键元数据。
- `data-block-type` 使用原始下划线形式的 `BlockType` 值，是新版 DocVortex HTML 的唯一机器类型来源；
  `docvortex-caption`、`docvortex-footnote` 和 `docvortex-figure--*` 等 class 只负责样式。
...
- 新版公式 carrier 保存 `data-formula-display` 与 `data-docvortex-latex`。网页公式按固定来源优先级收敛为裸 LaTeX：
...
- 新版 DocVortex HTML 可按类型级往返；FULL 中页面辅助类型可恢复，但 HTML 输入仍固定投影为一个逻辑页，
...
2026-09-06 标记迁移：精确路径仅识别 DocVortex v1 标记；此前生成的 MinerU 标记 HTML
