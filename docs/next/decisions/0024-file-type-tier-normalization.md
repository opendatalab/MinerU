...
PDF 和 image 支持质量 tier: `flash`、`basic`、`standard`、`advanced`。Office、HTML 和 CSV 当前只有 `flash` 语义，没有对应的质量 tier。其它 text 可入库和搜索，但不需要解析。此前不同入口对“非 PDF/image + 质量 tier”的处理不完全一致:
...
Office、HTML 和 CSV 即使通过高层接口传入了 `basic`、`standard` 或 `advanced`，也不能把结果记录为这些质量 tier，除非未来另有 ADR 明确引入对应能力。text 不创建 parse row 或 Middle JSON，显式 parse 返回 `parse_not_required`。
...
批量操作中，Office/HTML/CSV 的实际 parse row、缓存目录、locator 和 metadata 必须记录 `flash`，不能记录用户传入的质量 tier。其它 text 不得生成这些解析产物。

API Server 当前只有 job 级 `tier` 字段，暂不扩展 file-level effective tier 字段。因此 API job 响应中的 job tier 可以表示请求 tier；但内部执行 Office/HTML/CSV 文件时，应按 `flash` 语义处理，不产生伪质量缓存。其它 text 不进入 API parse job。
...
- Office、HTML 和 CSV 命中 parsing-rule 时，忽略 rule 中的 `tier` 和 `remote`，按 `flash` 解析。
...
| 入口 | Office/HTML/CSV + remote | text + remote |
|------|----------------------|---------------|
...
`mineru-kit parse` 是低层工具，允许 `--remote` 处理 Office/HTML/CSV；当这些输入不能使用质量 tier 时，按批量归一规则处理。其它 text 不作为解析输入。

## 替代方案

### 方案 A: Office/HTML/CSV 一律静默忽略 tier
...
### 方案 B: Office/HTML/CSV 一律遇到质量 tier 报错
...
- 批量任务经常包含混合文件类型。
- parsing-rule 是后台批量策略，不应因为 Office/HTML/CSV 携带无效质量 tier 而阻断整个目录处理。
...
- doclib 单文件主动解析保持严格语义: Office/HTML/CSV 显式质量 tier 报错；其它 text 返回 `parse_not_required`。
- Office/HTML/CSV 未指定 tier 或指定 `flash` 时，实际记录为 `flash`。
- parsing-rule 触发 Office/HTML/CSV 时，实际 parse row 必须为 `flash`；其它 text 只入库和索引。
...
- `mineru-kit parse --remote` 对 Office/HTML/CSV 是允许的低层工具特例；其它 text 不作为解析输入。

### 对 API Server

- API Server job 按批量操作语义处理。
- Office/HTML/CSV 即使 job tier 是质量 tier，执行时也按 `flash` 语义；其它 text 不进入 parse job。
- 暂不新增 file-level effective tier 字段。

### 对缓存和 locator

- 缓存键、parse row、locator 和产物 metadata 必须记录实际 tier。
- Office/HTML/CSV 不应产生 `basic`、`standard` 或 `advanced` 缓存；其它 text 不应产生任何解析缓存。

## 后续动作

1. 更新 doclib parsing-rule 入队逻辑，确保 Office/HTML/CSV 创建 `tier=flash` 的 parse row，其它 text 不创建 parse row。
