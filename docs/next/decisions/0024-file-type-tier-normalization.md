...
MinerU 同时支持 PDF、image、Office、text 和 HTML 输入，但这些输入并不具备相同的 tier 能力。

PDF 和 image 支持质量 tier: `flash`、`medium`、`high`、`xhigh`。Office、text 和 HTML 当前只有 `flash` 语义，没有对应的质量 tier。此前不同入口对“非 PDF/image + 质量 tier”的处理不完全一致:

- 有的入口报错。
- 有的入口静默走 Office / text / HTML parser。
...
MinerU 在 tier 语义上把输入分为两组:

| 分组 | 文件类型 | Tier 能力 |
|------|----------|-----------|
| 支持多 tier 的输入 | PDF、image | `flash`、`medium`、`high`、`xhigh` |
| 仅支持 flash tier 的输入 | Office、text、HTML | 仅 `flash` 语义 |

Office、text 和 HTML 即使通过高层接口传入了 `medium`、`high` 或 `xhigh`，也不能把结果记录为这些质量 tier，除非未来另有 ADR 明确引入对应能力。
...
| Office / text / HTML | 归一为 `flash` | 使用 `flash` | 报错 |

因此，单对象主动操作中，用户显式指定质量 tier 表达明确质量预期。对于没有质量 tier 能力的输入，系统不能静默降级。
...
批量操作的目标是处理一组异构文件。为了避免一个 Office、text 或 HTML 文件阻断整个批次，仅支持 flash tier 的输入允许静默归一为 `flash`。
...
| Office / text / HTML | 归一为 `flash` | 使用 `flash` | 归一为 `flash` |

批量操作中，非 PDF/image 的实际 parse row、缓存目录、locator 和 metadata 必须记录 `flash`，不能记录用户传入的质量 tier。

API Server 当前只有 job 级 `tier` 字段，暂不扩展 file-level effective tier 字段。因此 API job 响应中的 job tier 可以表示请求 tier；但内部执行非 PDF/image 文件时，应按 `flash` 语义处理，不产生伪质量缓存。
...
- Office、text 和 HTML 命中 parsing-rule 时，忽略 rule 中的 `tier` 和 `remote`，按 `flash` 解析。
...
| 入口 | 非 PDF/image + remote |
|------|----------------------|
| doclib / `mineru parse` 单文件主动解析 | 报错 |
| parsing-rule | 忽略 remote，按 `flash` |
| API Server job | 不适用；API Server 自身就是解析服务 |
| `mineru-kit parse` | 允许，作为低层工具特例 |
...
`mineru-kit parse` 是低层工具，允许 `--remote` 处理非 PDF/image 输入；当非 PDF/image 不能使用质量 tier 时，按批量归一规则处理。

## 替代方案

### 方案 A: Office/text/HTML 一律静默忽略 tier
...
### 方案 B: Office/text/HTML 一律遇到质量 tier 报错
...
- 批量任务经常包含混合文件类型。
- parsing-rule 是后台批量策略，不应因为非 PDF/image 文件携带无效质量 tier 而阻断整个目录处理。
...
- doclib 单文件主动解析保持严格语义: 非 PDF/image 显式质量 tier 报错。
- 非 PDF/image 未指定 tier 或指定 `flash` 时，实际记录为 `flash`。
- parsing-rule 触发非 PDF/image 时，实际 parse row 必须为 `flash`。
- parsing-rule 对 PDF/image 未指定 tier 时，允许 `high -> xhigh -> medium -> flash` 兜底。

### 对 SDK 和 mineru-kit

- SDK 单文件解析应按单对象主动操作语义处理。
- `mineru-kit parse` 需要根据展开后的输入数量区分单对象和批量。
- `mineru-kit parse --remote` 对非 PDF/image 是允许的低层工具特例。

### 对 API Server

- API Server job 按批量操作语义处理。
- 非 PDF/image 文件即使 job tier 是质量 tier，执行时也按 `flash` 语义。
- 暂不新增 file-level effective tier 字段。

### 对缓存和 locator

- 缓存键、parse row、locator 和产物 metadata 必须记录实际 tier。
- 非 PDF/image 不应产生 `medium`、`high` 或 `xhigh` 缓存。

## 后续动作

1. 更新 doclib parsing-rule 入队逻辑，确保非 PDF/image 始终创建 `tier=flash` 的 parse row。
