...
PDF 和 image 支持质量 tier: `flash`、`basic`、`standard`、`advanced`。Office 和 HTML 当前只有 `flash` 语义，没有对应的质量 tier。text 可入库和搜索，但不需要解析。此前不同入口对“非 PDF/image + 质量 tier”的处理不完全一致:
...
Office 和 HTML 即使通过高层接口传入了 `basic`、`standard` 或 `advanced`，也不能把结果记录为这些质量 tier，除非未来另有 ADR 明确引入对应能力。text 不创建 parse row 或 Middle JSON，显式 parse 返回 `parse_not_required`。
...
- parsing-rule 对 PDF/image 未指定 tier 时，允许 `standard -> advanced -> basic -> flash` 兜底。
...
- Office/HTML 不应产生 `basic`、`standard` 或 `advanced` 缓存；text 不应产生任何解析缓存。

## 后续动作

1. 更新 doclib parsing-rule 入队逻辑，确保 Office/HTML 创建 `tier=flash` 的 parse row，text 不创建 parse row。
2. 更新 parsing-rule 的 PDF/image 默认 tier 选择顺序为 `standard -> advanced -> basic -> flash`。
