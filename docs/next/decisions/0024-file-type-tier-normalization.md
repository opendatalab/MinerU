...
PDF 和 image 支持质量 tier: `flash`、`basic`、`standard`、`advanced`。EPUB、Office、HTML 和 CSV 当前只有 `flash` 语义，没有对应的质量 tier。其它 text 可入库和搜索，但不需要解析。此前不同入口对“非 PDF/image + 质量 tier”的处理不完全一致:

- 有的入口报错。
- 有的入口静默走 Office / HTML / CSV parser，或为其它 text 返回没有对应缓存的合成成功响应。
- parsing-rule 可能创建 `tier=standard` 等 parse row，但实际执行的仍是 Office / flash 语义。

这种不一致会让 parse row、缓存目录、locator 和用户看到的实际解析能力脱节。因此需要定义一套按操作类型区分的归一化规则。

本 ADR 细化并部分修订 ADR-0022。ADR-0022 中 doclib 主动单文件解析的严格错误语义仍保留；本 ADR 补充 batch、parsing-rule、API Server 和 mineru-kit 的例外。ADR-0028 进一步把 CSV 从无需解析 text 移入仅支持 flash tier 的结构化输入组。
...
| 分组 | 文件类型 | Tier 能力 |
|------|----------|-----------|
| 支持多 tier 的输入 | PDF、image | `flash`、`basic`、`standard`、`advanced` |
| 仅支持 flash tier 的输入 | Office、HTML、CSV | 仅 `flash` 语义 |
| 无需解析的文本输入 | `.txt`、`.md`、`.markdown`、`.rst`、`.tex` | 无 tier；直接读取源文件 |

EPUB、Office、HTML 和 CSV 即使通过高层接口传入了 `basic`、`standard` 或 `advanced`，也不能把结果记录为这些质量 tier，除非未来另有 ADR 明确引入对应能力。text 不创建 parse row 或 Middle JSON，显式 parse 返回 `parse_not_required`。
...
| 输入类型 | 未指定 tier | 指定 `flash` | 指定 `basic/standard/advanced` |
|----------|-------------|--------------|--------------------------|
| PDF / image | 使用该入口默认质量选择策略 | 使用 `flash` | 使用指定质量 tier |
| Office / HTML / CSV | 归一为 `flash` | 使用 `flash` | 报错 |
...
| Office / HTML / CSV | 归一为 `flash` | 使用 `flash` | 归一为 `flash` |
| text | 不进入 parse 批次 | 不进入 parse 批次 | 不进入 parse 批次 |

批量操作中，EPUB/Office/HTML/CSV 的实际 parse row、缓存目录、locator 和 metadata 必须记录 `flash`，不能记录用户传入的质量 tier。其它 text 不得生成这些解析产物。

API Server 当前只有 job 级 `tier` 字段，暂不扩展 file-level effective tier 字段。因此 API job 响应中的 job tier 可以表示请求 tier；但内部执行 EPUB/Office/HTML/CSV 文件时，应按 `flash` 语义处理，不产生伪质量缓存。其它 text 不进入 API parse job。
...
- EPUB、Office、HTML 和 CSV 命中 parsing-rule 时，忽略 rule 中的 `tier`、`page_range` 和 `remote`，按 `flash` 整本解析。
...
| 入口 | EPUB/Office/HTML/CSV + remote | text + remote |
|------|----------------------|---------------|
...
`mineru-kit parse` 是低层工具，允许 `--remote` 处理 EPUB/Office/HTML/CSV；当这些输入不能使用质量 tier 时，按批量归一规则处理。其它 text 不作为解析输入。

## 替代方案

### 方案 A: EPUB/Office/HTML/CSV 一律静默忽略 tier
...
### 方案 B: EPUB/Office/HTML/CSV 一律遇到质量 tier 报错
...
- 批量任务经常包含混合文件类型。
- parsing-rule 是后台批量策略，不应因为 EPUB/Office/HTML/CSV 携带无效质量 tier 而阻断整个目录处理。
...
- doclib 单文件主动解析保持严格语义: EPUB/Office/HTML/CSV 显式质量 tier 报错；其它 text 返回 `parse_not_required`。
- EPUB/Office/HTML/CSV 未指定 tier 或指定 `flash` 时，实际记录为 `flash`。
- parsing-rule 触发 EPUB/Office/HTML/CSV 时，实际 parse row 必须为 `flash`；其它 text 只入库和索引。
- parsing-rule 对 PDF/image 未指定 tier 时，允许 `standard -> basic -> flash` 兜底。

### 对 SDK 和 mineru-kit

- SDK 单文件解析应按单对象主动操作语义处理，并排除 text 解析输入。
- `mineru-kit parse` 需要根据展开后的输入数量区分单对象和批量。
- `mineru-kit parse --remote` 对 EPUB/Office/HTML/CSV 是允许的低层工具特例；其它 text 不作为解析输入。

### 对 API Server

- API Server job 按批量操作语义处理。
- EPUB/Office/HTML/CSV 即使 job tier 是质量 tier，执行时也按 `flash` 语义；其它 text 不进入 parse job。
- 暂不新增 file-level effective tier 字段。

### 对缓存和 locator

- 缓存键、parse row、locator 和产物 metadata 必须记录实际 tier。
- EPUB/Office/HTML/CSV 不应产生 `basic`、`standard` 或 `advanced` 缓存；其它 text 不应产生任何解析缓存。

## 后续动作

1. 更新 doclib parsing-rule 入队逻辑，确保 EPUB/Office/HTML/CSV 创建 `tier=flash` 的 parse row，其它 text 不创建 parse row。
