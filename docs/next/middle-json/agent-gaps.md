...
| chunk id | 暂缓 | hash 型 chunk id 可作为后续补充，不作为 P0 主引用。 |
| locator | 已决策，未实现 | 已有统一 page/block 地址格式，但代码尚未落地。 |
...
hash 型 chunk id 暂不作为 P0 主引用。P0 以 `block_ref` 作为 Agent citation 的主 ID。

后续如果需要不可枚举 ID、跨 schema major version 隔离或更短 token，可增加函数:

```text
chunk_id = hash(schema_major, file_sha256, tier, block_ref)
```
...
| `tier` | 产生该结果的 tier。 |
| `block_ref` | P0 主 block reference。 |

原因:

- `file_sha256` 保证同名不同文件不冲突。
- `block_ref` 已包含 short id、tier、页号和块号，便于回查。
...
3. 按需增加 hash 型 `chunk_id`。
4. 如未来需要 child/list item locator，另行定义公开契约。
