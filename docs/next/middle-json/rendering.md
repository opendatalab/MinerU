...
EPUB 固定使用共享 planner 的 DEFAULT 连续语义，在单个 `article.docgale-document` 中展平页面，
...
`article.docgale-document` fragment，不包含 CSS、脚本或
`head`。显式标题缺失时，HTML title 使用首个非空 `doc_title` 的纯文本，再回退为
`DocGale Document`。完整文档使用 `body.docgale-html-body` 提供居中和响应式页面布局；
fragment 调用方可以自行决定外围容器尺寸。

HTML 与 Markdown 共用 `RenderMode` 和续段/续表 planner。DEFAULT 输出无页面 wrapper
的连续阅读内容；FULL 为每个 `PageInfo` 输出一个 `section.docgale-page`，包括空页，并在
相邻页面之间输出 `hr.docgale-page-break`。每个顶层 block wrapper 保留来源 page/type/index
元数据。`article.docgale-document` 同时声明 `data-docgale-html-version="1"` 和
...
`docgale-math` carrier 内使用 `\(...\)` 或 `\[...\]`，不扫描普通正文中的美元符号。每个公式
carrier 同时保存 `data-block-type="equation"`、`data-formula-display` 和裸 LaTeX
`data-docgale-latex`，供 HTML parser 无损恢复；可见 MathJax 定界符不属于 Middle JSON 内容。
...
完整文档只内联压缩后的 `docgale.min.css`。其可读源码为 `docgale.css`，所有文档规则均
限定在 `.docgale-document` 内，standalone 外围布局规则限定在 `.docgale-html-body`，
...
fragment 调用方需要加载随包提供的 `docgale.min.css`，并按实际内容加载同版本
...
- `page_footnote`: 独立于页面辅助块，Markdown/HTML 的 DEFAULT/FULL 以及固定默认 DOCX/EPUB/PDF 都输出；Markdown 使用无可见标签的 `<small><span class="docgale-page-footnote" data-block-type="page_footnote" style="color:#6b7280">…</span></small>`，其余格式使用各自的脚注样式和 anchor。
