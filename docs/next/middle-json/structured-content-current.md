...
| 调用点 | 当前产物名 |
|--------|------------|
| `ParseResult.save()` | `{prefix}_structured_content.json` |
| 旧 CLI dump | `{pdf_file_name}_structured_content.json` |
| client-side output regeneration | `{doc_stem}_structured_content.json` |
| 当前 parse API server | `output_files.structured_content` |

这些是实现现状。NEXT 版公开 API / CLI / SDK 统一使用 `structured_content` / `structured-content`。
...
当前 PDF backend 的 structured_content item bbox 是归一化到 0-1000 的整数框:
...
- Office 通常不输出 bbox。
- structured_content converter 没有统一过滤 `EMPTY_BBOX`；如果 source bbox 是 `(0, 0, 0, 0)`，当前可能输出 `[0, 0, 0, 0]`。
...
- `md` 和 `code_inline` 当前没有在主要 structured_content converter 中稳定产出。
...
Office structured_content 直接使用 `asdict(span)` 输出 span，因此字段更接近 Middle JSON 的 `Span`:
...
- Office index item 复用 `_flatten_structured_list_items()`，因此可能带 `ilevel`、`prefix`、`anchor`。
