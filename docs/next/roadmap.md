...
- 支持可寻址 chunk id，便于 citation 和回溯。
...
解析能力通过 `flash`、`standard`、`pro` 暴露；用户未指定 tier 时使用默认选择策略。tier 的用户语义、硬件边界和质量策略见 [解析 Tier](tiers.md)。
...
这些能力依赖 middle_json 的最小统一子集：稳定 chunk id、page、bbox 和 type。
