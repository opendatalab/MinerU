...
对 PDF/image，省略 `tier` 或传 `null` 时，本地 server 按 [解析 Tier](../tiers.md) 的默认选择策略选择非 `flash` 质量 tier。如果不可用，请求应失败，而不是使用 `flash` 返回解析结果。EPUB/Office/HTML/CSV 按 [ADR-0024](../decisions/0024-file-type-tier-normalization.md) 与 [ADR-0028](../decisions/0028-csv-structured-flash-parsing.md) 的 API Server 批量规则归一为 `flash`，其它 text 不作为解析输入。
