...
| `tier` | string 或 null | 否 | `null` | `standard`、`pro` 或 `null`。省略或传 `null` 表示使用默认选择策略，选择当前可发现的最高质量非 `flash` tier。HTML 输入自动路由到 HTML 解析器。 |
...
| 503 | `quality_tier_unavailable` | 请求的 `standard` / `pro` 不可用，或默认选择策略找不到可用的非 `flash` tier。 |
...
- 省略 `tier` 或传 `null` 时，只能选择本地可发现的 `standard` 或 `pro`，不能回退到 `flash`。
