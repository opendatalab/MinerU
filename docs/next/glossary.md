...
| Doclib SDK | `MineruClient` | 连接本地 doclib 的产品 SDK | v1 API client |
...
| `standard` | 实体 tier | 面向普通本地硬件的模型解析能力 |
| `pro` | 实体 tier | 当前最高质量解析能力，可来自 `mineru.net/api` 或高端本地硬件 |

规范:

- CLI/API/SDK 的用户可见参数应使用 `tier`。
- CLI 不传 `--tier`、HTTP API 省略 `tier` 或传 JSON `null`、Python SDK 传 `None` 表示使用默认选择策略。
- 任务入队、缓存目录、产物 metadata 应记录实际使用的实体 tier，即 `flash`、`standard` 或 `pro`。
- 默认选择策略永远不能解析为 `flash`。
...
它接收文件或本地路径，执行 `standard` / `pro` 等解析能力，并返回解析任务与产物。它不管理长期文档库、watch、搜索索引或用户本地配置。
...
它是 v1 Unified API 的主线定义来源，也是当前 `pro` tier 的默认远端能力来源。
...
主动阅读场景下，未指定 tier 应使用默认选择策略，并至少解析到 `standard`。
...
| Pro backend | `pro` tier 或具体 backend | `pro` 是解析档位，不是解析后端名称 |
