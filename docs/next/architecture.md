...
通用文档引擎已抽为独立依赖 `docvortex`。MinerU 保留应用、推理和任务管理，原生解析、PDF 分类与基础访问、文档类型、确定性后处理、素材和 renderer 由 DocVortex 唯一维护。

```text
MinerU Flash TXT ──→ DocVortex 原生分析 ──→ ModelJson
...
PDF 只有 `auto` 模式调用共享 classify；显式 `txt` 不分类，显式 `ocr` 保留已有推理路线。原生分析只由 Flash 分支调用。DocVortex 不反向依赖 MinerU。

DocVortex 内存对象和 JSON 使用中性 producer/extensions；MinerU 对外继续输出 schema 2.0，由兼容 codec 恢复产品元数据。所有持久化边界应使用 ParseResult 或明确的 MinerU codec，而不是直接写出 DocVortex 对象的原生封装。
...
| 边界 | 选择 | 目的 |
|------|------|------|
| CLI/MCP/桌面端 -> doclib | HTTP + JSON over UDS or TCP loopback | UDS 优先；Windows Python 无 UDS 时自动 fallback 到 TCP |
...
| 通道 | 地址 | 用途 |
|------|------|------|
| UDS | `$MINERU_HOME/doclib.sock` | CLI、MCP、桌面端访问 doclib，UDS 可用时默认启用 |
| TCP | `127.0.0.1:15980` | UDS 不可用时访问 doclib 的本地 fallback |
| endpoint discovery | `$MINERU_HOME/doclib.endpoint.json` | CLI / SDK 发现 doclib 实际可用 transport |
| TCP | `127.0.0.1:16580` | doclib 调用 local parse-server |
...
当前 doclib HTTP API 统一挂在 `/api/v1` 前缀下。对外稳定边界以 doclib public models 和 SDK client route 为准，主要资源包括 `/server/status`、`/parses`、`/invalidate`、`/forget`、`/scans`、`/files`、`/docs`、`/search`、`/find`、`/configs`、`/watches`、`/exclude-rules`、`/parsing-rules`、`/cleanup/*` 和 `/telemetry/*`。
...
| 条件 | 执行路径 |
|------|----------|
| 未指定 tier | PDF/image 通过当前目标 parse-server 能力发现，按 `standard` -> `basic` 解析；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`；其它 text 直接读取 |
...
解析产物按 `sha256`、实际使用的 `tier` 和解析页码批次隔离存储。doclib 的 `parsed/` 目录持久化 Middle JSON 批次文件，并可保存解析结果引用到的图片 sidecar：
...
当前代码中的 watch discoverable 文件类型包括：`pdf`、`ofd`、`epub`、`doc`、`docx`、`ppt`、`pptx`、`xls`、`xlsx`、`rtf`、`odt`、`ods`、`odp`、`html`、`htm`、`shtml`、`mhtml`、`mht`、`csv`、`tsv`、`md`、`markdown`、`rst`、`tex`、`txt`。图片不在 watch 白名单中，但可以通过显式 `mineru parse image.png` 触发。Apple 文档、其它电子书和邮件格式暂不在当前 discoverable 白名单内。
...
| OFD / EPUB / Office / HTML / MHTML / CSV / TSV | 本地 CPU Flash 整本解析；显式 page_range 拒绝 |
