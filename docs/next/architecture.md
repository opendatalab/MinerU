...
| 条件 | 执行路径 |
|------|----------|
| 未指定 tier | PDF/image 通过当前目标 parse-server 能力发现，按 `standard` -> `basic` 解析；OFD/EPUB/Office/HTML/CSV/TSV 归一为 `flash`；其它 text 直接读取 |
...
当前代码中的 watch discoverable 文件类型包括：`pdf`、`ofd`、`epub`、`doc`、`docx`、`ppt`、`pptx`、`xls`、`xlsx`、`rtf`、`odt`、`ods`、`odp`、`html`、`htm`、`csv`、`tsv`、`md`、`markdown`、`rst`、`tex`、`txt`。图片不在 watch 白名单中，但可以通过显式 `mineru parse image.png` 触发。Apple 文档、其它电子书和邮件格式暂不在当前 discoverable 白名单内。
...
| OFD / EPUB / Office / HTML / CSV / TSV | 本地 CPU Flash 整本解析；显式 page_range 拒绝 |
