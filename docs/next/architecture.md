...
| 通道 | 地址 | 用途 |
|------|------|------|
| UDS | `/tmp/mineru.sock` | CLI、MCP、桌面端访问 doclib |
...
| `parses` | 一次解析请求 | 记录 tier、pages、privacy、状态和错误 |
...
| `scan_status` | `active` / `deleted` / `unreachable` |
| `locked_at` | IngestWorker 抢占锁 |
| `error_code` / `error_msg` | 文件级错误 |

`scan_status` 表达路径可达性，而不是 doc 生命周期:
...
文件可以来自 watch，也可以来自显式 `mineru parse <path>`、`mineru info <path>`、`GET /docs?path=...` 或 `invalidate(path=...)`。

所有 source file path 入口在使用 `files.sha256` 前，都必须先执行统一文件发现/刷新步骤:

1. 按扩展名白名单和 exclude 规则过滤。
2. 对 path 执行轻量 stat，读取 `size_bytes`、`mtime_ms` 和 `birthtime_ms`。
...
6. `IngestWorker` 通过原子 SQL 抢占 `scan_status=active AND sha256 IS NULL` 的文件。
...
- 标记 `deleted` 或 `unreachable` 时不清理 FTS；默认 search / find 通过 `scan_status='active'` 控制可见性。
...
4. 查询 `(sha256, tier)` 下已完成批次，判断 pages 是否覆盖请求范围。
...
doclib 当前推荐使用 SQLite `config` 表作为配置源，由 `mineru config` 管理，不引入外部配置文件。

基础配置：

| key | 默认值 | 说明 |
|-----|--------|------|
| `data_dir` | `~/MinerU` | 数据目录 |
| `watch_default_tier` | `flash` | watch 自动解析默认 tier |
...
- deleted file cleanup 只删除 `scan_status='deleted'` 的 file row，并清理对应 `fts_filenames`。
...
- `parses.error_*`: 这一次 tier/pages parse batch 有问题。
