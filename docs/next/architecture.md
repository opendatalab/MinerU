...
当前代码中的 watch discoverable 文件类型包括：`pdf`、`ofd`、`epub`、`doc`、`docx`、`ppt`、`pptx`、`xls`、`xlsx`、`rtf`、`odt`、`ods`、`odp`、`html`、`htm`、`csv`、`md`、`markdown`、`rst`、`tex`、`txt`。图片不在 watch 白名单中，但可以通过显式 `mineru parse image.png` 触发。Apple 文档、其它电子书和邮件格式暂不在当前 discoverable 白名单内。
...
| OFD / EPUB / Office / HTML / CSV | 本地 CPU Flash 整本解析；显式 page_range 拒绝 |
