...
- 默认选择 / `flash` / `medium` / `high` / `xhigh` 的语义以 [解析 Tier](tiers.md) 为准。
...
| parse-server | 解析服务 | 无状态解析服务，提供 v1 Unified API，执行 `medium` / `high` / `xhigh` 等质量 tier。 |
...
文件来源
  ...
  │   -> doclib ParseService
  │   -> 查缓存
  │   -> 默认选择/flash/medium/high/xhigh 路由
  │   -> ParseWorker
...
|------|-----------|
| watch 自动发现 | `flash` |
| parsing-rule 明确指定 | PDF/image 使用 rule 中的 tier；Office/text/HTML 归一为 `flash` |
| parsing-rule 未指定 tier | PDF/image 按 `high` -> `xhigh` -> `medium` -> `flash`；Office/text/HTML 归一为 `flash` |
...
主动读取 PDF/image 的目标是尽可能准确地理解文档。未指定 tier 时，使用默认选择策略，并解析到可用的非 `flash` 质量 tier。Office/text/HTML 没有质量 tier，未指定 tier 时归一为 `flash`。
...
6. PDF/image 默认选择策略通过当前可用 parse-server 能力发现，按 `high` -> `xhigh` -> `medium` 选择；Office/text/HTML 归一为 `flash`。
...
| 请求 | 实际行为 |
|------|----------|
| 未指定 tier | PDF/image 按 `high` -> `xhigh` -> `medium` 选择可用的非 `flash` tier；Office/text/HTML 归一为 `flash` |
...
| `tier=xhigh` | 使用本地或自部署 parse-server 的 `xhigh` 能力 |
...
2. 客户端创建 parse job。
3. 请求可以携带 `tier`；省略或传 JSON `null` 时，PDF/image 使用默认选择策略，Office/text/HTML 按批量规则归一为 `flash`。
...
- 可发现 Local Parse Server 支持 `high`。
