...
6. `IngestWorker` 抢占 `scan_status=active AND sha256 IS NULL` 的文件。
...
13. 将本批次的 Middle JSON pages 写入 `parsed/<sha-prefix>/<sha>/<tier>/<pages>_<done_at>.json`。
...
| 文件信息 | `GET /info?path=...` / SDK `get_file_info()` / `mineru info <path>` |
| 按路径列文档 | `GET /docs?path=...` / SDK `list_docs(path=...)` |
...
| 阶段 | 主要状态 |
|------|----------|
| 发现路径 | `files.scan_status=active` |
| 检测到变化 | 更新 `files.size_bytes/mtime_ms`，并设置 `files.sha256=NULL` |
| 文件缺失且设备可达 | `files.scan_status=deleted`，保留 `sha256`，写入 `deleted_at` |
| removable 设备不可达 | 后台 `DeviceMonitor` 可将 `watch_targets.watch_status=unreachable`，并批量把相关 active files 变为 `unreachable`；同步 path 调用只允许更新当前文件和 watch 状态 |
...
1. scan 开始时先检查 watch root 是否可达；不可达时只更新 `watch_targets.watch_status=unreachable` 并返回。
2. root 可达时，先读取该 watch 下 `scan_status=active` 的已知 file paths，并逐个调用统一文件刷新逻辑。
...
- 手动命令立即删除所有 `scan_status=deleted` 的 file row。
...
8. 查询 `(sha256, tier, pages)` 是否已有可复用结果。
...
```text
<!-- mineru:next pages=6~10 tier=pro -->
```
...
页码范围由 `parses.pages` 和对应 JSON 文件共同表达。请求某个页码范围时，doclib 会:

1. 查询同一 `sha256 + tier` 下已完成批次。
2. 忽略已经 invalidate 的批次。
3. 忽略 JSON 文件已经丢失的批次。
4. 用有效已完成批次的 `pages` 覆盖集合判断请求页码是否已经满足。
...
8. 解析产物按 `sha256 + tier + pages + done_at` 写入 JSON。
9. 主动阅读不会静默命中 `flash`。
...
4. 任务完成后写入 `parsed/<sha-prefix>/<sha>/flash/<pages>_<done_at>.json`。
5. 搜索可以命中文档。
...
3. JSON 产物写入 `parsed/<sha-prefix>/<sha>/standard/<pages>_<done_at>.json`。
