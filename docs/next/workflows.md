...
|------|-----------|
| watch 自动发现 | `flash` |
| parsing-rule 明确指定 | PDF/image 使用 rule 中的 tier；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`；其它 text 只入库和索引 |
| parsing-rule 未指定 tier | PDF/image 按 `standard` -> `basic` -> `flash`；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`；其它 text 只入库和索引 |
...
主动读取 PDF/image 的目标是尽可能准确地理解文档。未指定 tier 时，使用默认选择策略，并解析到可用的非 `flash` 质量 tier。OFD/EPUB/Office/HTML/MHTML/CSV/TSV 没有质量 tier，未指定 tier 时归一为 `flash`。其它 text 应直接读取，不进入解析流程。
...
6. PDF/image 默认选择策略通过当前可用 parse-server 能力发现，按 `standard` -> `basic` 选择；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`；其它 text 直接读取。
...
| 请求 | 实际行为 |
|------|----------|
| 未指定 tier | PDF/image 按 `standard` -> `basic` 选择可用的非 `flash` tier；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 归一为 `flash`；其它 text 直接读取 |
...
6. PDF/image 请求未指定 tier 时使用默认选择策略；OFD/EPUB/Office/HTML/MHTML/CSV/TSV 按 `flash` 语义处理；其它 text 不发起 parse 请求。
...
2. 客户端创建 parse job。
3. 请求可以携带 `tier`；省略或传 JSON `null` 时，PDF/image 使用默认选择策略，OFD/EPUB/Office/HTML/MHTML/CSV/TSV 按批量规则归一为 `flash`；其它 text 不作为解析输入。
