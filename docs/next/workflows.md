...
| parsing-rule 未指定 tier | PDF/image 按 `high` -> `xhigh` -> `medium` -> `flash`；Office/HTML 归一为 `flash`；text 只入库和索引 |
...
6. PDF/image 默认选择策略通过当前可用 parse-server 能力发现，按 `high` -> `xhigh` -> `medium` 选择；Office/HTML 归一为 `flash`；text 直接读取。
...
| 请求 | 实际行为 |
|------|----------|
| 未指定 tier | PDF/image 按 `high` -> `xhigh` -> `medium` 选择可用的非 `flash` tier；Office/HTML 归一为 `flash`；text 直接读取 |
