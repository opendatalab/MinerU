...
范围: `flash`、`medium`、`high`、`extra_high` 解析 tier 的产品语义、默认选择策略、适用场景、质量边界和执行路径
...
| `extra_high` | 专家最高档 | 本地或自部署高质量档位，面向更高成本和更强硬件 |
...
| 维度 | `flash` | `medium` | `high` | `extra_high` |
|------|---------|------------|--------|--------------|
| 主要目标 | 快速扫大量文件，建立基础索引 | 在普通本地硬件上提供可接受的高质量解析 | 提供高质量解析 | 提供最高成本/最高质量档位 |
| 底层原理 | 纯 CPU 实现 | 基于模型 | 基于更强模型 | 基于更高 effort 的模型路径 |
| 质量 | 低 | 中 | 高 | 更高 |
| 速度 | 最快 | 中等 | 较慢，取决于硬件和远端服务 | 最慢，取决于硬件和远端服务 |
| 本地硬件要求 | 无 GPU / 加速器也可用 | 需要至少 16GB 总内存；GPU 可选，纯 CPU 可用但可能较慢 | 需要至少 16GB 总内存，并具备符合条件的本地加速器，或使用远端 | 同 `high`，通常需要更强硬件或更长等待 |
...
MinerU 有 local parse-server，也有 remote parse-server。无论 local 还是 remote，parse-server 都应支持通过 API 发现当前 server 提供了哪些解析 tier。当前代码支持发现 `medium`、`high`、`extra_high` 等质量 tier。
...
| 只支持 `medium` | `medium` |
| 只支持 `high` | `high` |
| 只支持 `extra_high` | `extra_high` |
| 同时支持 `medium` 和 `high` | `high` |
| 同时支持 `medium`、`high` 和 `extra_high` | `extra_high` |
| 只支持 `flash` 或没有可用能力 | 报错 |

默认选择策略永远不会解析为 `flash`。如果系统找不到 `medium`、`high` 或 `extra_high`，应返回 `quality_tier_unavailable` 或等价结构化错误，而不是擅自用 `flash` 内容替代。错误码见 [错误码体系](errors.md)。

默认选择策略是请求时的选择逻辑，不是长期缓存语义。任务入队、缓存命中、产物目录和结果 metadata 应使用实际选择的实体 tier（例如 `medium`、`high` 或 `extra_high`），避免默认选择与实体 tier 产生重复缓存。
...
P0 不做基于启发式的自动提示或自动排队升级。watch 使用 `flash` 后，如果需要在后台自动解析到 `medium`、`high` 或 `extra_high`，必须来自用户显式配置的 parsing-rules；系统不根据文件名、内容特征、搜索命中或“高价值文档”判断自行升级。
...
`medium` 是本地主力解析 tier，当前使用 `hybrid-engine` 的 `medium` effort。它是一系列本地模型串联的解析方案。
...
本地 managed `medium`、`high` 和 `extra_high` parse server 都要求至少 16GB 总内存。低于该基线时，不建议启用本地 managed 质量 tier；应考虑远端解析，或在用户明确接受低质量时显式使用 `flash`。
...
`extra_high` 是当前公开 tier 中最高的本地/自部署档位，映射到 `hybrid-engine` 的 `extra_high` effort。

它适合极高质量要求、专家批处理或自部署服务。相对 `high`，它可能带来更长等待时间、更高显存/内存压力和更高运行成本。用户主动指定 `--tier extra_high`，或目标 parse-server 能力发现只暴露/优先暴露 `extra_high` 时，系统可以使用该 tier。
...
用户或 Agent 主动读取文档时，默认应解析到可用的非 `flash` 质量 tier。

如果 `medium`、`high` 和 `extra_high` 都不可用，且远端也不可用，系统应报错，而不是擅自把 `flash` 结果作为最终阅读结果返回。
...
| 用户主动指定 `--tier extra_high` | 本地或自部署 extra_high；不可用时报错 |
...
- 公开 tier 取值包括 `flash`、`medium`、`high`、`extra_high`。
- CLI 不传 `--tier`、HTTP API 省略 `tier` 或传 JSON `null`、Python SDK 传 `None`，表示使用默认选择策略。
- 默认选择策略必须通过 parse-server 能力发现解析为非 `flash` 质量 tier。
- 默认阅读质量不能静默降级到 `flash`。
