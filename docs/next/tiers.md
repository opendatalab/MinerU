...
```text
standard -> advanced -> basic -> 报错
```

也就是说，`standard` 是默认优先档位；`advanced` 虽然质量最高，但不会在 `standard` 同时可用时自动成为默认。只有没有 `standard`、但有 `advanced` 时，默认才选择 `advanced`。

| 可发现能力 | 默认解析结果 |
|------------|-----------------|
| 只支持 `basic` | `basic` |
| 只支持 `standard` | `standard` |
| 只支持 `advanced` | `advanced` |
| 同时支持 `basic` 和 `standard` | `standard` |
| 同时支持 `standard` 和 `advanced` | `standard` |
| 同时支持 `basic` 和 `advanced`，不支持 `standard` | `advanced` |
| 同时支持 `basic`、`standard` 和 `advanced` | `standard` |
| 只支持 `flash` 或没有可用能力 | 报错 |

PDF/image 默认选择策略不会解析为 `flash`。如果系统找不到 `basic`、`standard` 或 `advanced`，应返回 `quality_tier_unavailable` 或等价结构化错误，而不是擅自用 `flash` 内容替代。错误码见 [错误码体系](errors.md)。
