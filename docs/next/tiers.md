...
这类用户可能拥有 GPU 或加速器，但显存和算力有限。它们不一定完全跑不动 `high`，但运行 `high` 通常会很慢。因此 `medium` 是一个算力消耗较低、质量中等、仍然基于模型的解析方案。

`medium` 的核心价值是本地隐私解析：当用户不接受把文档发送到远端解析，同时硬件又不是特别高端时，`medium` 应成为本地可用的最佳默认方案。

`medium` 的最低硬件基线按 Hybrid low 本地小模型执行定义:
...
`high` 默认代表最高质量解析能力。它既可以由 `mineru.net/api` 提供，也可以由本地高端硬件运行。拥有高端 NVIDIA 数据中心或工作站级计算卡的用户，应能通过本地 parse-server 正式运行这个 tier。
