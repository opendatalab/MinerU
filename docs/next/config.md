...
- 质量优先：PDF/image 主动阅读未指定 tier 时使用默认选择策略，不能静默降级到 `flash`；Office/HTML/CSV 按实际能力归一为 `flash`；其它 text 直接读取。
...
- 对 Office/HTML/CSV，parsing-rule 的 tier 和 remote 不生效，实际按 `flash` 记录；其它 text 只入库和索引，不创建 parse row。
