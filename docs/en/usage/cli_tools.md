...
- `MINERU_CPU_NUM_THREADS`: upper bound on the CPU threads one torch CPU operator may use in the API server; default `8`, clamped to the machine's core count. CPU work is dispatched with `asyncio.to_thread()`, and libgomp keeps one persistent worker team per thread that enters a parallel region, so without a bound the process thread count grows towards `thread_pool_size * cpu_count` on many-core hosts. Skipped entirely when `OMP_NUM_THREADS`, `MKL_NUM_THREADS` or `OPENBLAS_NUM_THREADS` is already set; falls back to `MINERU_INTRA_OP_NUM_THREADS` when unset.
