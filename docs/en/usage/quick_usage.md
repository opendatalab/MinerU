...
```
> [!TIP]
>- `<input_path>`: Local `PDF` / image / `CSV` / `RTF` / `DOC`/`DOCX` / `PPT`/`PPTX` / `XLS`/`XLSX` / `ODT`/`ODS`/`ODP` file or directory
...
- FastAPI calls:
  ...
  >- Uploads currently support `PDF`, image, `CSV`, `RTF`, `DOCX`, `PPTX`, `XLSX`, `ODT`, `ODS`, and `ODP` files
  >
