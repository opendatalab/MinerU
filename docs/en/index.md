...
MinerU is a document parsing tool that converts `PDF`, `EPUB`, static `HTML`, image, `CSV`, `RTF`, `DOCX`, `PPTX`, `XLSX`, `ODT`, `ODS`, and `ODP` inputs into machine-readable formats such as Markdown and JSON for downstream retrieval, extraction, and processing.
...
- Support `PDF`, `EPUB`, static `HTML`, image, `CSV`, `RTF`, `DOCX`, `PPTX`, `XLSX`, `ODT`, `ODS`, and `ODP` inputs
