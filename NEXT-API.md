...
>
> **核心约定**:API Key 仅影响配额、并发、队列优先级与可选项,**不影响请求/响应格式**。
...
| 2026-05-27 | **Access Level 重命名**:`Free` → `anonymous`(无 API Key),`Pro` → `registered`(有 API Key),两档不变 |
...
| 2026-05-27 | **限流粒度确定**:per-key、per-minute、按端点类别(parse/upload/chat/read)四档分桶,独立限额(§11.1) |
...
| 2026-05-27 | **匿名用户身份与权限**:anonymous 按 IP 标识(限流/用量),`GET /v1/files` 和 `GET /v1/parse/jobs` 返回 `403 list_requires_api_key`(§1.3/§5.8/§8.4/§11) |
...
与 OpenAI 兼容的统一错误结构,HTTP 状态码 + `type`(大类) + `code`(细分):
...
| `message` | string | ✅ | 人类可读描述,包含上下文和修复建议 |
...
| type | 含义 |
|------|------|
| `invalid_request_error` | 请求参数非法、资源不存在等客户端错误 |
| `authentication_error` | API Key 无效或过期 |
| `permission_error` | 权限不足(配额耗尽、功能需认证等) |
| `rate_limit_error` | 限流 |
| `engine_error` | 解析引擎相关错误(无引擎、引擎崩溃、解析失败) |
| `api_error` | 服务端内部错误 |

所有错误响应均携带 `X-Request-Id` 头。
...
| 400 | `invalid_request_error` | `invalid_sha256sum` | sha256sum 格式非法(非 64 位小写 hex) |
| 400 | `invalid_request_error` | `file_hash_mismatch` | 上传字节实际 SHA-256 与申请时不一致 |
| 400 | `invalid_request_error` | `bytes_mismatch` | 实际上传字节数与创建时声明的 `bytes` 不一致 |
| 401 | `authentication_error` | `invalid_api_key` | API Key 无效或过期 |
| 403 | `permission_error` | `quota_exceeded` | 配额耗尽 |
| 403 | `permission_error` | `feature_requires_api_key` | 功能需认证 |
| 403 | `permission_error` | `list_requires_api_key` | 无权访问列表接口 |
| 404 | `invalid_request_error` | `job_not_found` / `file_not_found` / `upload_not_found` / `model_not_found` | 资源不存在 |
| 409 | `invalid_request_error` | `job_already_terminal` | 状态不可变更(如取消已完成任务) |
| 409 | `invalid_request_error` | `upload_not_ready` | 调用 Complete 时字节尚未上传 |
| 413 | `invalid_request_error` | `file_too_large` | 文件超出当前等级限制 |
| 429 | `rate_limit_error` | `rate_limit_exceeded` | 触发限流 |
| 500 | `api_error` | `internal_error` | 服务端错误 |
| 503 | `api_error` | `service_unavailable` | 模型不可用,可重试 |

> 完整错误码体系详见 NEXT-ERROR.md。
...
- `403 feature_requires_api_key`:模型存在但当前 access level 无权使用(响应体含 `upgrade_url`)
...
成功返回 `200 OK`。无需 Bearer API Key,预签名 URL 自带授权。
...
> anonymous 用户调用此端点返回 `403 list_requires_api_key`,需 Bearer API Key 才能列出文件列表。
...
> anonymous 用户调用此端点返回 `403 list_requires_api_key`,需 Bearer API Key 才能列出任务列表。
...
Access Level 由 API Key 决定,但**响应格式完全一致**。

| 能力 | anonymous(无 API Key) | registered(有 API Key) |
|------|---------------------|-----------------------|
| 单文件页数 | ≤ 1000 | ≤ 1000 |
| 单文件大小 | ≤ 200 MB | ≤ 200 MB |
| 单任务文件数 | 100 | 100 |
...
超限时返回 `403 feature_requires_api_key` 或 `413 file_too_large`,响应体包含升级提示。
...
限流粒度为 **per-minute**，按端点类别独立计数。registered 用户按 **API Key** 计数，anonymous 用户按 **请求 IP** 计数。
...
- registered:按 API Key 统计用量;anonymous:按请求 IP 统计用量
- 不传 API Key 也能查,返回 anonymous 档的 `limits`
...
| `POST /parse/file` (Flash) | `POST /v1/uploads` → `PUT` → `POST /v1/uploads/{id}/complete` → `POST /v1/parse/jobs`(无 API Key) |
...
统一为单个 Parser 类,基址 + 可选 API Key 决定档位:
...
调用代码完全相同,API Key 仅决定能不能用 `tier="pro"` 或 `output_formats=["docx"]`。
...
- 旧:`extra_formats` / 文档格式由 API Key 等级隐式控制;新:`output_formats` 统一收纳,高级格式(`docx`/`html`/`latex`)需 API Key,无权限返回 `403 feature_requires_api_key`
