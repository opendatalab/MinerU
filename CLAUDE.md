...
| 环境 | 用途 | 冲突原因 |
|---|---|---|
| ① MinerU 主环境 | 跑解析、测试、demo | 锁定 `vllm<0.22.0`、`transformers<5` |
| ② OvisOCR2 服务环境 | `vllm serve` 起识别服务 | 要求 `vllm==0.22.1`，与 ① 冲突 |
| ③ transformers-5 环境（可选） | PP-DocLayoutV3 出 layout.json | HF 移植版要求 `transformers>=5`，与 ① 冲突 |
...
使用 transformers 官方移植版 `PaddlePaddle/PP-DocLayoutV3_safetensors`
（输出自带阅读顺序；注意其 id2label 有歧义，脚本内按 cls_id 映射，勿改动）：
