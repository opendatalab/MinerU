...
PUBLIC_OCR_LANGUAGE_CHOICES = tuple(f"{lang} ({_PUBLIC_OCR_LANGUAGE_DESCRIPTIONS[lang]})" for lang in PUBLIC_OCR_LANGUAGES)
...
def format_public_ocr_lang_description() -> str:
    ...
    option_lines = [f"- {lang}: {_PUBLIC_OCR_LANGUAGE_DESCRIPTIONS[lang]}." for lang in PUBLIC_OCR_LANGUAGES]
    return (
...
def validate_public_ocr_lang(lang: str) -> str:
    ...
    if normalized_lang not in PUBLIC_OCR_LANGUAGES:
        raise ValueError(f"Language {lang} not supported. Allowed values: " + ", ".join(PUBLIC_OCR_LANGUAGES))
