...
def _get_first_non_whitespace_char(text: str) -> str | None:
...
def _get_last_non_whitespace_char(text: str) -> str | None:
...
def _replace_ascii_spaces_with_marker(
    ...
    def flush_text_buffer() -> None:
        if text_buffer:
            rendered_parts.append(_escape_office_inline_text("".join(text_buffer), inline_syntax))
            text_buffer.clear()
...
def _render_text_with_edge_space_markers(
    ...
    return marker * leading_space_count + _escape_office_inline_text(core_text, inline_syntax) + marker * trailing_space_count
...
def _iter_para_inline_spans(para_block: Block) -> Generator[Span, None, None]:
...
def _select_block_inline_syntax(para_block: Block) -> str:
...
def _extend_style_range_tokens(
    ...
    if marker:
        ...
        if marker == "-" and not _is_ascii_space_only(original_content):
            ...
            ignored_style = "underline" if marker == "_" else "strikethrough"
            render_style = [name for name in render_style if name != ignored_style]
        else:
            rendered_content = _render_text_with_edge_space_markers(
                original_content,
                marker,
...
def _append_hyperlink_part(
    ...
    if children:
        ...
        if not styled_text:
            return
        rendered_content = styled_text if plain_text_only else _render_link(styled_text, url, inline_syntax)
    else:
...
def merge_para_with_text(para_block: Block, escape_text_block_prefix: bool = True) -> str:
    """将 Office 段落 block 渲染为 Markdown/HTML 混合文本，按 block 自动选择行内语法。"""
    inline_syntax = _select_block_inline_syntax(para_block)
    parts = []
    text_span_buffer = []

    def flush_text_span_buffer() -> None:
