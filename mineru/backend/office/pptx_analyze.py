...
from ...model.flash import PptxModel
from ...types import PageInfo
from ...utils.image_payload import ImagePayloadCache
from .model_output_to_middle_json import result_to_middle_json


def office_pptx_analyze(file_bytes: bytes, image_cache: ImagePayloadCache | None = None) -> tuple[list[PageInfo], list[Any]]:
    infer_start = time.time()

    file_stream = BytesIO(file_bytes)
    results = PptxModel().predict(file_stream)
...
if __name__ == "__main__":
    ...
    with open(pptx_path, "rb") as f:
        file_bytes = f.read()
    middle_json, results = office_pptx_analyze(file_bytes)
