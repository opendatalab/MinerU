...
def __is_list_or_index_block(block: Block) -> str:
                    ...
                    for i, line in enumerate(block.lines):
                        if len(lines_text_list[i]) > 0 and lines_text_list[i][-1] in LIST_END_FLAG:
                            ...
                            if i + 1 < len(block.lines):
                                block.lines[i + 1]._is_list_start = True
        ...
        if len(text_blocks_group) > 1:
            ...
            for i in range(len(text_blocks_group) - 1, -1, -1):
                ...
                if i - 1 >= 0:
                    ...
                    if current_block.type == BlockType.VERTICAL_TEXT and prev_block.type == BlockType.VERTICAL_TEXT:
                        __merge_2_vertical_text_blocks(current_block, prev_block)
                    elif current_block.type == BlockType.TEXT and prev_block.type == BlockType.TEXT and not is_list_group:
                        __merge_2_text_blocks(current_block, prev_block)
                    elif (current_block.type == BlockType.LIST and prev_block.type == BlockType.LIST) or (
