...
class BatchAnalyze:
    ...
    def __call__(self, images_with_extra_info: list) -> list:
        ...
        # doclayout_yolo
        layout_images = []
        for _image_index, image in enumerate(images):
        ...
        if self.formula_enable:
            ...
            for _index in range(len(images)):
                images_layout_res[_index] += images_formula_list[_index]
                mfr_count += len(images_formula_list[_index])
        ...
        for _index in range(len(images)):
            pil_img, ocr_enable, _lang = images_with_extra_info[_index]
            layout_res = images_layout_res[_index]
        ...
        if self.enable_ocr_det_batch:
            ...
            for lang, lang_crop_list in lang_groups.items():
                ...
                for crop_info in lang_crop_list:
                    ...
                    _group_key = (normalized_h, normalized_w)
                    resolution_groups[_group_key].append(crop_info)

                # 对每个分辨率组进行批处理
                for _group_key, group_crops in tqdm(
                    ...
                    # 批处理检测
                    batch_size = min(
                        len(batch_images), self.batch_ratio * 16
                    )  # 增加批处理大小
                    ...
                    # 处理批处理结果
                    for _i, (crop_info, (dt_boxes, _elapse)) in enumerate(
