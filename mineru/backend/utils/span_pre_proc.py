...
def txt_spans_extract(
    ...
    try:
        ...
        if page_char_count is not None and page_char_count > MAX_NATIVE_TEXT_CHARS_PER_PAGE:
            ...
            need_ocr_spans = [span for span in spans if span.get("type") == ContentType.TEXT]
        ...
        if len(vertical_spans) > 0:
            page_all_lines = [
                line for line in get_lines_from_chars(page_chars["chars"]) if _is_supported_rotation(line["rotation"])
            ]
...
def _prepare_post_ocr_spans(
    need_ocr_spans: list[dict[str, Any]],
    spans: list[dict[str, Any]],
    pil_img: Any,
    scale: int,
) -> list[dict[str, Any]]:
...
class SpanBlockMatcher:
    ...
    @staticmethod
    def _get_grid_size(spans: list[dict[str, Any]]) -> int:
        """根据 span 高度估算索引网格大小，避免过细或过粗。"""
        heights = [
            span["bbox"][3] - span["bbox"][1] for span in spans if span.get("bbox") and span["bbox"][3] > span["bbox"][1]
        ]
        if not heights:
            return 1
        return max(1, statistics.median(heights))

    def _build_grid(self, spans: list[dict[str, Any]]) -> dict[int, list[int]]:
    ...
    def _cell_range(self, bbox: list[float]) -> tuple[int, int]:
...
def chars_to_content(span: dict[str, Any]) -> None:
    # 检查span中的char是否为空
    if len(span["chars"]) != 0:
        chars = span["chars"]
