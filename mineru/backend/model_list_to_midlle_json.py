...
def _validate_page_index_map(
    ...
    if len(page_index_map) != page_count:
        raise ValueError(
            f"page_index_map length mismatch: pages={page_count}, mapping={len(page_index_map)}"
        )
    if any(
        isinstance(page_idx, bool) or not isinstance(page_idx, int) or page_idx < 0
        for page_idx in page_index_map
    ):
    ...
    if any(
        current <= previous
        for previous, current in zip(page_index_map, page_index_map[1:])
    ):
...
def _blocks_to_raw_page_info(
    ...
    *,
    ...
    """运行单页 MagicModel 并保留 raw dict，供跨页后处理继续消费。"""
    magic_model = MagicModel(page_model_list, use_bbox=use_bbox)
...
def blocks_to_page_info(
    ...
    *,
    ...
    if not resolved_use_bbox:
        MagicModel.fix_office_paragraph_titles([copied_blocks])
...
def model_list_to_pages(
    ...
    if not use_bbox:
        MagicModel.fix_office_paragraph_titles(copied_model_list)
