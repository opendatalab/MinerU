...
for _dirs in [
    ...
    # 尝试获取语言列表
    if _dirs in locals():
        lang_list = locals()[_dirs]
        # 如果语言列表不为空，且不是特殊标记语言，则添加到all_lang中
        if lang_list and lang_list[0] not in ["ch", "en"]:
            all_lang.extend(lang_list)
    ...
    **kwargs,
    ...
    if sglang_engine_enable:
        try:
            ...
            # 初始化模型但不需要使用返回值
            model_singleton.get_model("sglang-engine", None, None, **kwargs)
