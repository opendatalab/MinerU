...
from .enum_class import ContentType
from .ocr_utils import _is_overlaps_x_exceeds_threshold, _is_overlaps_y_exceeds_threshold

VERTICAL_SPAN_HEIGHT_TO_WIDTH_RATIO_THRESHOLD = 2
VERTICAL_SPAN_IN_BLOCK_THRESHOLD = 0.8


def is_vertical_text_block_by_spans(spans: list[dict[str, Any]]) -> bool:
...
def fix_text_block(block: dict[str, Any]) -> dict[str, Any]:
...
def merge_spans_to_vertical_line(spans: list[dict[str, Any]], threshold: float = 0.6) -> list[list[dict[str, Any]]]:
...
def line_sort_spans_by_left_to_right(lines: list[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    line_objects = []
    for line in lines:
        #  按照x0坐标排序
...
def vertical_line_sort_spans_from_top_to_bottom(vertical_lines: list[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    line_objects = []
