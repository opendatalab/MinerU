...
def sort_lines_by_model(fix_blocks, page_w, page_h, line_height, footnote_blocks):
    ...
    for left, top, right, bottom in page_line_list:
        ...
        assert 1000 >= right >= left >= 0 and 1000 >= bottom >= top >= 0, (
...
def cal_block_index(fix_blocks, sorted_bboxes):
    if sorted_bboxes is not None:
        ...
        for _i, block in enumerate(fix_blocks):
...
def revert_group_blocks(blocks):
    ...
    for _group_id, blocks in image_groups.items():
        new_blocks.append(
            process_block_list(blocks, BlockType.IMAGE_BODY, BlockType.IMAGE)
        )

    for _group_id, blocks in table_groups.items():
        new_blocks.append(
