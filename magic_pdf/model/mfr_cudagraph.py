...
class PatchedMBartDecoder(nn.Module):
    def __init__(self, origin: nn.Module, kvlen: torch.LongTensor):
        ...
        self.counting_context_weight = origin.counting_context_weight
...
class GraphRunner(nn.Module):
    ...
    def create_buffers(self, encoder_kvlens: int, dtype: torch.dtype):
        ...
        for _ in range(decoder_layers):
            k0 = torch.zeros((max_batchs, num_heads, max_kvlens, head_dim), dtype=dtype, device=device)
            v0 = torch.zeros_like(k0)
            k1 = torch.zeros((max_batchs, num_heads, encoder_kvlens, head_dim), dtype=dtype, device=device)
