...
class CustomTorroneModel:
    ...
    def __init__(self, ocr: bool = False, show_log: bool = False, **kwargs):
        ...
        if self.apply_ocr:
            ...
            model = TorroneModel.from_pretrained("0xCarbon/torrone2-base-10ep")
            self.torrone_model = move_to_device(model, bf16=not False, cuda=(self.device=="cuda"))
        ...
        if self.apply_ocr:
            ...
            for res in layout_res:
                if int(res['category_id']) in [0, 1, 2, 4, 6, 7]:  # 需要进行ocr的类别
                    ...
                    # masked_img = Image.new('RGB', pil_img.size, 'white')
                    ...
                    model_output = self.torrone_model.inference(
                        image_tensors=sample, early_stopping=False #skipping
                    )
